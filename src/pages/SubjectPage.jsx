...
// We now import the new 'getFilesByField' function
...
const SubjectPage = () => {
  ...
  useEffect(() => {
    const fetchSubjectFiles = async () => {
      setLoading(true);
      try {
        // We format the subject name and call getFilesByField
        const subjectName = subjectId.replace(/-/g, ' ');
        const fileData = await getFilesByField('subject', subjectName);
        setFiles(fileData);
      } catch (error) {
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-gray-50 rounded-lg">
          <p className="text-lg text-gray-500">
            No files are available for this subject yet.
          </p>
